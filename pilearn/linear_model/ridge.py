...
class PiRidgeRegression(PiBaseRegression):
    ...
    The ridge coefficients minimize a penalized residual sum of squaresself.

    Here 'alpha' >= 0 is a complexity parameter that controls the amount of shrinkage.
    the larger the value of 'alpha' the greater the amount of shrinkage. This makes the coefficients more robust to collinearity.
    """
    ...
    def fit(self, alpha=1.):
        ...
        """
        super().fit()
        self.coefficients = np.linalg.inv(self.X.T.dot(self.X) + np.identity(self.n_features).dot(alpha)).dot(self.X.T.dot(self.y))
